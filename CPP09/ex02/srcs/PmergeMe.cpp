...
PmergeMe::PmergeMe(int nb_args, char **args)
{
    for (int i = 0; i < nb_args; i++)
    {
        ...
        if (std::find(this->_vector.begin(), this->_vector.end(), std::atoi(args[i])) != this->_vector.end())
            continue;
        this->_vector.push_back(std::atoi(args[i]));
        this->_set.insert(std::atoi(args[i]));
    }
    this->_valid = true;
}
...
void PmergeMe::sort()
{
    ...
    clock_t v_end = clock();
    double v_duration = static_cast<double>(v_end - v_start) / CLOCKS_PER_SEC;

    clock_t s_start = clock();
    std::set<int> sorted_set = sort_set(this->_set.begin(), this->_set.end());
    clock_t s_end = clock();
    double s_duration = static_cast<double>(s_end - s_start) / CLOCKS_PER_SEC;

    print_before();
    print_after(sorted_vector);

    std::cout << "Time to process a range of " << this->_vector.size() << " elements with std::vector -> " << v_duration << "s" << std::endl;
    std::cout << "Time to process a range of " << this->_vector.size() << " elements with std::set    -> " << s_duration << "s" << std::endl;
